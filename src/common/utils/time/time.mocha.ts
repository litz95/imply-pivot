...
describe('Time', () => {
  ...
  it('gets human friendly end time which is -1 ms from actual end time', () => {
    ...
    var endWallTimeInclusive = (getEndWallTimeInclusive(endExclusive, timezone) as any).format('YYYY-MM-DD[T]HH:mm:ss.SSS');
    expect(endWallTimeInclusive, 'tijuana').to.equal("1995-03-08T15:59:59.999");

    endExclusive = new Date("1995-03-09T00:00:00.000Z");
    endWallTimeInclusive = (getEndWallTimeInclusive(endExclusive, TZ_KATHMANDU) as any).format('YYYY-MM-DD[T]HH:mm:ss.SSS');
    expect(endWallTimeInclusive, 'kathmandu').to.equal("1995-03-09T05:44:59.999");

    endExclusive = new Date("1999-03-09T00:00:00.000Z");
    endWallTimeInclusive = (getEndWallTimeInclusive(endExclusive, TZ_TIJUANA) as any).format('YYYY-MM-DD[T]HH:mm:ss.SSS');
    expect(endWallTimeInclusive, 'tijuana2').to.equal("1999-03-08T15:59:59.999");

    endExclusive = new Date("2016-02-28T00:00:00.000Z");
    endWallTimeInclusive = (getEndWallTimeInclusive(endExclusive, TZ_Kiritimati) as any).format('YYYY-MM-DD[T]HH:mm:ss.SSS');
